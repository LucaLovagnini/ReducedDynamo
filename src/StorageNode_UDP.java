		...
		public void run () {
			...
			try {
					...
					while (true) {
							...
							for (int i = 0;i < list.size();i++) { //estraggo l'i-esimo pacchetto dalla lista
									...
									if (type.equals("node")) {
											...
											//estraggo dalla nodeTable IP e porta TCP del successore
											...
											//estraggo dalla nodeTable IP e porta TCP del predecessore
									...
									//se mi ha contattato un client devo contattare (tramite una nuova Socket TCP) il nodo che detiene il dato cercato
									else { //ottengo l'id del dato che vuole il client e il suo indirizzo ip + porta udp
											...
											nodeTable = multicast.getTable();
											//stabilisco quale nodo detiene il dato cercato dal client
											Enumeration <Integer> items = nodeTable.keys();
