...
public class StorageNode implements Runnable {
	...
	if(bootstrapServerIp==null)//versione locale
		...
		try {nodeIp=InetAddress.getLocalHost().getHostAddress();
			} 
	...
	try
		...
		String ipBN = serverObject.start_join_node(nodeIp);//qui non c'è distinzione tra locale e rete
		if(ipBN.equals("-1"))
		...
		if(ipBN.contains(" bootstrapnode"))
			...
			if(bootstrapServerIp!=null)	//versione di rete
				KA = new StorageNode_KeepAlive(nodeIp,bootstrapServerIp,scanner);//si occuperà di inviare al BootstrapServer_KeepAlive i propri segnali
			else
		...
		if(!ipBN.equals("1"))
			...
			do
				...
				//la stringa type indica che è un nodo a contattatare il nodo di bootstrap
				String type = "node";
				dos.writeUTF(type);
