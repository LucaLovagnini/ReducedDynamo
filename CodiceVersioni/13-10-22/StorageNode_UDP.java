...
public class StorageNode_UDP extends Thread {
	...
	public void run ()
		...
		StorageNode_UDPWriter UDPW = new StorageNode_UDPWriter(this,UDPM,ds,list,ds.getLocalPort());
		try
			{UDPW.start();
			while(true)
				{UDPM.StartRead();
				...
				for(int i=0;i<list.size();i++)
					...
					if(type.equals("node"))
						...
						System.out.println("file_id ricevuto="+file_id);
					...
					//se mi ha contattato un client invece devo contattare (tramite il mio TCP) il nodo che detiene il dato cercato
						...
						nodeTable=multicast.getTable();
						//stabilisco quale nodo DOVREBBE detenere il dato cercato dal client
						Enumeration <Integer> items = nodeTable.keys();
						...
						int detentore=-1;
						//caso in cui il dato è detenuto è detenuto dal nodo più piccolo
						...
						String fakeipDetentore = indirizziDetentore.substring(start,end);//mi ricavo l'indirizzo ip del detentore del dato
