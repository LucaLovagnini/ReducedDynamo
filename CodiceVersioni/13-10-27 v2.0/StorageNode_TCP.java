...
public class StorageNode_TCP extends Thread{
	...
	StorageNode_TCPWriter TCPR;//il thread che gestirà il run di questo oggetto si occuperà di gestire le richieste in ingresso
	...
	public void run()
		...
		//questa sleep proporzionale al node_number garantisce che la stampa dei contenuti della mia tabella dati non interferisca con quella degli altri nodi
		try
		...
		while(true)
			{try
				...
				TCPM.StartRead();
				//System.out.println("Tocca al Reader");
				...
				for(int i=0;i<list.size();i++)
					...
					//ma una socket TCP creta dall'oggetto che gestisce l'UDP del nodo di bootstrap che si è occupato del client
						...
						if(network==0)
							...
							{ia=InetAddress.getByName(fakeipClient);}
