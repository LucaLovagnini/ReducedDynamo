...
public class StorageNode implements Runnable {
	...
	try
		...
		System.out.println("ipBN= "+ipBN);
		//se il nuovo nodo è un novo di bootstrap
		if(ipBN.contains(" bootstrapnode"))
			{StorageNode_KeepAlive KA;
			if(bootstrapServerIp!=null)	//versione di rete
				KA = new StorageNode_KeepAlive(fakeip,bootstrapServerIp,scanner);//si occuperà di inviare al BootstrapServer_KeepAlive i propri segnali
			else
				KA = new StorageNode_KeepAlive(fakeip,scanner);
			KA.start();
		...
		if(!ipBN.equals("1"))
			...
			ipBN=ipBN.substring(end_node_number+4, ipBN.length());//elimino il nodenuber dalla stringa ipBN
			}
		else
			node_number=1;
		...
		if(bootstrapServerIp!=null)	//versione di rete
			multicast = new StorageNode_Multicast(nodeTable,"id="+file_id+" IP="+fakeip,1);
		else						//versione locale
			multicast = new StorageNode_Multicast(nodeTable,"id="+file_id+" IP="+fakeip,0);
		multicast.start();
		...
		if(bootstrapServerIp!=null)
			TCP = new StorageNode_TCP(Thread.currentThread(),TCPServiceSocket,predIP,succIP,fakeip,predPort,succPort,file_id,dataTable,node_number,multicast,scanner,1);
		else
			TCP = new StorageNode_TCP(Thread.currentThread(),TCPServiceSocket,predIP,succIP,fakeip,predPort,succPort,file_id,dataTable,node_number,multicast,scanner,0);
		try
		{TCP.start();
