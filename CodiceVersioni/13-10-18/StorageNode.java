...
public class StorageNode implements Runnable {
	...
	try
		...
		//altrimenti devo contattare il nodo di bootstrap
			...
			//viene inviato anche i propri indirizzi per permettere di ricevere una risposta
			dos.writeUTF(fakeip);
			...
			System.out.println("succIP="+succIP+" succPort="+succPort+" predIP="+predIP+" predPort="+predPort);
			//...e la tabella dei nodi presenti nella rete
			nodeTable = (Hashtable<Integer,String>) oin.readObject();
			//System.out.println("predIP="+predIP+" succIP="+succIP+" predPort="+predPort+" succPort="+succPort);
			}
		...
		StorageNode_UDP UDP = new  StorageNode_UDP(UDPServiceSocket,multicast);
		try
		{UDP.start();
		UDP.join();
		}
		...
		StorageNode_TCP TCP = new StorageNode_TCP(Thread.currentThread(),TCPServiceSocket,predIP,succIP,fakeip,predPort,succPort,file_id,dataTable,node_number);
		try
		{TCP.start();
